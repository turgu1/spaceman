class SpecialtiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @specialty.save
        format.html { redirect_to @specialty, notice: t('.create_ok') }
  ...
  def update
    respond_to do |format|
      if @specialty.update(specialty_params)
        format.html { redirect_to @specialty, notice: t('.update_ok') }
  ...
  def destroy
    @specialty.destroy
    respond_to do |format|
      format.html { redirect_to specialties_url, status: 303 }
      format.json { head :no_content }
    end
  end
